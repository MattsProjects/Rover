...
namespace MFConsoleApplication1
{
    ...
    public class C328R
    {
        ...
        public enum SnapshotType { Compressed = 0, Uncompressed = 1 };
        ...
        /// <returns>True if succeeded</returns>
        public bool Snapshot(SnapshotType snapshotType, int skipFrameCounter)
        {
        ...
        private bool WaitForResponse(ref byte[] readBuffer)
        {
            int recv = _serialPort.Read(readBuffer, 0, readBuffer.Length);
            return recv != 0;
        }
        ...
        /// See C328R user manual for more information about error codes.
        /// Error codes:
        ...
        private bool ReceiveACK(byte expectedACKCommand)
        {
            byte[] responseBuffer = new byte[6];
            bool stat = WaitForResponse(ref responseBuffer);
        ...
        private bool SendCommand(byte[] commandArray)
        {
            ...
            int send = _serialPort.Write(commandArray, 0, len);            
            Thread.Sleep(10);
